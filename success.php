...
$cart=mysqli_query($db, "SELECT * FROM cart where email='$email'") or die(mysqli_error());
...
$prodname=$row['name'];
$prodprice=$row['price'];	
$order=mysqli_query($db, "INSERT INTO `ecommerce`.`order` (`email`, `product_id`, `name`, `quantity`, `price`, `img`) VALUES('$email','$pi','$prodname','$pqty', '$prodprice','$pimg')") or die(mysqli_error());

}

$un=mysqli_query($db, "DELETE FROM cart WHERE email='$email' and product_id='$pi'") or die(mysqli_error());
